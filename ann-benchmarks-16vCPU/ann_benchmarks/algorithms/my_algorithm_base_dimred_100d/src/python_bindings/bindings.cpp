...
class Index {
 public:
    ...
    void init_new_index(
        ...
        cur_l = 0;
        appr_alg = new hnswlib::HierarchicalNSW<dist_t>(l2space, maxElements, M, efConstruction, random_seed, allow_replace_deleted, alpha);
        index_inited = true;
        ep_added = false;
    ...
    void enableDimReduction(size_t full_dim, const std::vector<size_t>& target_dims, 
                           int threshold_level = 1, std::string reduction_type = "random") {
        if (!appr_alg) {
            throw std::runtime_error("Index must be initialized before enabling dimensionality reduction");
        }
        
        appr_alg->enableDimensionalityReduction(full_dim, target_dims, threshold_level, reduction_type);
    }

    // Build dimensionality reduction after adding data
    void buildDimReduction() {
        if (!appr_alg) {
            throw std::runtime_error("Index must be initialized before building dimensionality reduction");
        }
        
        appr_alg->buildDimensionalityReduction();
    }

    // >>> REMOVED Incorrectly added C++ method definition here <<<
    ...
    py::class_<Index<float>>(m, "Index")
        ...
        .def("init_index",
             [](Index<float> & self,
                ...
                bool allow_replace_deleted,
                float alpha) {
                 self.init_new_index(maxElements, M, efConstruction, random_seed, allow_replace_deleted, alpha);
             },
             py::arg("max_elements"),
             py::arg("M"),
        ...
        .def("get_num_layers", [](const Index<float>& index) {
            if (!index.index_inited || !index.appr_alg) {
                throw std::runtime_error("Index is not initialized.");
            }
            return index.appr_alg->getNumLayers();
        ...
        .def("enable_dim_reduction", &Index<float>::enableDimReduction,
            py::arg("full_dim"), py::arg("target_dims"), py::arg("threshold_level") = 1, py::arg("reduction_type") = "random")
        .def("build_dim_reduction", &Index<float>::buildDimReduction);
