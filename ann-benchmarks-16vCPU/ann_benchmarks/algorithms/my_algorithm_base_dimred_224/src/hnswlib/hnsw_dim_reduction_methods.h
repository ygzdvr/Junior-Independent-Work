...
void HierarchicalNSW<dist_t>::buildDimensionalityReduction() {
    ...
    // Create dimensionality reduction objects for each level
    for (int level = dim_reduction_threshold_level_; level <= maxlevel_; level++) {
        if (level - dim_reduction_threshold_level_ < (int)target_dims_.size()) {
            ...
            // Ensure DR object doesn't already exist
            if (dim_reduction_per_level_[level] != nullptr) {
                continue;
            }
            ...
            if (reduction_type_ == "pca") {
                dim_reduction_per_level_[level] = std::make_shared<PCAReduction>(training_data, target_dim);
            } else { // Default to random projection
                dim_reduction_per_level_[level] = std::make_shared<RandomProjection>(full_dim_, target_dim);
            }
