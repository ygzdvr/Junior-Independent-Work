...
#include "hnswlib.h"
#include "space_l2.h"    // Include existing spaces
#include "space_ip.h"
#include "space_opq.h"   // Include our new space
#include <thread>
#include <atomic>
#include <stdlib.h>
#include <assert.h>
...
class Index {
 public:
    ...
    size_t opq_m_ = 0; // ADDED: Store opq_m if used

    Index(const std::string &space_name, const int dim, size_t opq_m = 0) // ADDED opq_m parameter
        : space_name(space_name), dim(dim), opq_m_(opq_m) { // Store opq_m
        normalize = false;
        if (space_name == "l2") {
            if (opq_m > 0) {
                 if (dim % opq_m != 0) {
                     throw std::runtime_error("Dimension must be divisible by opq_m for OPQ");
                 }
                 std::cout << "Creating OPQSpace(dim=" << dim << ", opq_m=" << opq_m << ")" << std::endl;
                 l2space = new hnswlib::OPQSpace(dim, opq_m); // CREATE OPQSpace
            } else {
                 std::cout << "Creating L2Space(dim=" << dim << ")" << std::endl;
                 l2space = new hnswlib::L2Space(dim); // Original L2Space
            }
        } else if (space_name == "ip") {
             if (opq_m > 0) {
                 throw std::runtime_error("OPQSpace currently only supports L2 distance.");
             }
    ...
    void addItems(py::object input, py::object ids_ = py::none(), int num_threads = -1, bool replace_deleted = false) {
        if (opq_m_ > 0) {
             throw std::runtime_error("addItems cannot be used with OPQSpace. Use addItemsCodes instead.");
        }
    ...
    void addItemsCodes(py::object input, py::object ids_ = py::none(), int num_threads = -1, bool replace_deleted = false) {
        if (opq_m_ == 0) {
            throw std::runtime_error("addItemsCodes can only be used with OPQSpace.");
        }

        py::array_t<uint8_t, py::array::c_style | py::array::forcecast> codes(input);
        auto buffer = codes.request();
        if (num_threads <= 0)
            num_threads = num_threads_default;
        ...
        if (features != opq_m_) {
            char msg[256];
            snprintf(msg, sizeof(msg),
                "Wrong code dimensionality! Expected %zu, got %zu",
                opq_m_, features);
            throw std::runtime_error(msg);
        }
        if (normalize) {
             throw std::runtime_error("Normalization (cosine distance) is not supported with OPQSpace currently.");
        }
...

        {
            ...
            if (!ep_added) {
                // Add first point separately
                 size_t id = ids.size() ? ids.at(0) : (cur_l);
                 // Pass the code pointer directly
                 appr_alg->addPoint((void*)codes.data(0), (size_t)id, replace_deleted);
                 start = 1;
                 ep_added = true;
            }

            py::gil_scoped_release l;
             // Parallel addition of the rest
            ParallelFor(start, rows, num_threads, [&](size_t row, size_t threadId) {
                size_t id = ids.size() ? ids.at(row) : (cur_l + row);
                // Pass the code pointer for this row
                appr_alg->addPoint((void*)codes.data(row), (size_t)id, replace_deleted);
            });
    ...
    // ADDED: Binding for setting PQ centroids
    void set_pq_centroids(py::buffer centroids_buf) {
        if (!appr_alg) {
            throw std::runtime_error("Index is not initialized.");
        }
        py::buffer_info info = centroids_buf.request();
        if (info.format != py::format_descriptor<float>::format()) {
            throw std::runtime_error("Centroids buffer must be of type float32.");
        }
        // Basic dimension checks (can be more rigorous)
        if (info.ndim != 1 && info.ndim != 2) { // Allow flat or structured
             throw std::runtime_error("Centroids buffer must be 1D or 2D.");
        }

        const float* centroids_ptr = static_cast<const float*>(info.ptr);
        std::cout << "Bindings: Calling HierarchicalNSW::set_pq_centroids" << std::endl;
        appr_alg->set_pq_centroids(centroids_ptr);
        std::cout << "Bindings: HierarchicalNSW::set_pq_centroids returned" << std::endl;
    }
};
...
PYBIND11_PLUGIN(hnswlib) {
        ...
        .def(py::init<const std::string &, const int, size_t>(),
             py::arg("space"), py::arg("dim"), py::arg("opq_m") = 0) // Add opq_m optional arg
        .def("init_index",
        ...
        .def("add_items_codes", &Index<float>::addItemsCodes, // Bind addItemsCodes
             py::arg("codes"), py::arg("ids") = py::none(), py::arg("num_threads") = -1, py::arg("replace_deleted") = false)
        .def("set_pq_centroids", &Index<float>::set_pq_centroids, // Bind set_pq_centroids
             py::arg("centroids"))
        ...
        .def_property_readonly("ef_construction", [](const Index<float> &index) {
          return index.index_inited ? index.appr_alg->ef_construction_ : (size_t)0; // Or store efc in Index
        })
         .def_property_readonly("M", [](const Index<float> &index) {
             return index.index_inited ? index.appr_alg->M_ : (size_t)0; // Or store M in Index
         })
        .def_property_readonly("opq_m", [](const Index<float> &index) {
             return index.opq_m_;
         })
        .def("__len__", &Index<float>::getCurrentCount)
        .def(py::pickle(
