 ...
 public:
    ...
    py::object knnQuery_return_numpy(
...

        {
            ...
            std::fill_n(data_numpy_l, rows * k, -1); // Initialize with -1 (or appropriate sentinel)
            std::fill_n(data_numpy_d, rows * k, std::numeric_limits<float>::max());
            ...
            if (normalize == false) {
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    std::priority_queue<std::pair<float, hnswlib::labeltype >> result = appr_alg->searchKnn(
                        (void*)items.data(row), k, num_probes, p_idFilter);
                    
                    // Fill results, ensuring not to exceed k items even if search returns more due to ef > k
                    int result_idx = k - 1;
                    while (!result.empty() && result_idx >= 0) {
                        auto& result_tuple = result.top();
                        data_numpy_d[row * k + result_idx] = result_tuple.first;
                        data_numpy_l[row * k + result_idx] = result_tuple.second;
                        result.pop();
                        result_idx--;
                    }
                });
                ...
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    ...
                    std::priority_queue<std::pair<float, hnswlib::labeltype >> result = appr_alg->searchKnn(
                        (void*)(norm_array.data() + start_idx), k, num_probes, p_idFilter);
                    
                    // Fill results, ensuring not to exceed k items
                    int result_idx = k - 1;
                    while (!result.empty() && result_idx >= 0) {
                        auto& result_tuple = result.top();
                        data_numpy_d[row * k + result_idx] = result_tuple.first;
...

        {
            ...
            ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                std::priority_queue<std::pair<dist_t, hnswlib::labeltype >> result = alg->searchKnn(
                    (void*)items.data(row), k, 1, p_idFilter);
                for (int i = k - 1; i >= 0; i--) {
                    auto& result_tuple = result.top();
...
PYBIND11_PLUGIN(hnswlib) {
        ...
        .def("knn_query",
            [](Index<float> &index, py::object input, size_t k = 1, size_t num_probes = 1, int num_threads = -1, const std::function<bool(hnswlib::labeltype)>& filter = nullptr) {
                py::array_t < float, py::array::c_style | py::array::forcecast > items(input);
                auto buffer = items.request();
                hnswlib::labeltype* data_numpy_l;
                float* data_numpy_d;
                ...
                int M_num_threads = (num_threads <= 0) ? index.num_threads_default : num_threads;

                {
                    ...
                    // avoid using threads when the number of searches is small:
                    if (rows <= M_num_threads * 4) {
                        M_num_threads = 1;
                    }

                    data_numpy_l = new hnswlib::labeltype[rows * k];
                    data_numpy_d = new float[rows * k];
                    std::fill_n(data_numpy_l, rows * k, -1);
                    std::fill_n(data_numpy_d, rows * k, std::numeric_limits<float>::max());
                    ...
                    if (index.normalize == false) {
                        ParallelFor(0, rows, M_num_threads, [&](size_t row, size_t threadId) {
                            std::priority_queue<std::pair<float, hnswlib::labeltype >> result = index.appr_alg->searchKnn(
                                (void*)items.data(row), k, num_probes, p_idFilter);
                        ...
                        std::vector<float> norm_array(M_num_threads * index.dim);
                        ParallelFor(0, rows, M_num_threads, [&](size_t row, size_t threadId) {
                            size_t start_idx = threadId * index.dim;
                            index.normalize_vector((float*)items.data(row), (norm_array.data() + start_idx));
