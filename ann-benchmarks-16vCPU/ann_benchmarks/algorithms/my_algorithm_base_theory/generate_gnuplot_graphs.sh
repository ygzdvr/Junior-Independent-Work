...
# First run the Python converter to create gnuplot-friendly data files
...
# Get number of columns for EF values
ncols = system("awk 'NR==1 {print NF-1}' ${data_dir}/construction.dat")

# For O(n log n) reference curve
f(x) = a * x * log(x)

# Fit the curve using the last EF value
fit_col = ncols + 1
stats "${data_dir}/construction.dat" using 1:fit_col nooutput
a = column(fit_col)[STATS_records] / (STATS_max_x * log(STATS_max_x))

plot "${data_dir}/construction.dat" using 1:2 with linespoints pt 7 ps 0.7 title "EF=50", \\
EOF
        ...
        for ef in "${ef_values[@]}"; do
            if [ $col -le $(cat ${data_dir}/construction.dat | head -1 | wc -w) ]; then
                echo "     \"${data_dir}/construction.dat\" using 1:${col} with linespoints pt 7 ps 0.7 title \"EF=${ef}\", \\" >> "${output_dir}/gnuplot_construction.gp"
                col=$((col + 1))
            fi
        done
...
# Get number of columns for EF values
ncols = system("awk 'NR==1 {print NF-1}' ${data_dir}/search.dat")
...
# Fit the curve using the last EF value
fit_col = ncols + 1
stats "${data_dir}/search.dat" using 1:fit_col nooutput
b = column(fit_col)[STATS_records] / log(STATS_max_x)

plot "${data_dir}/search.dat" using 1:2 with linespoints pt 7 ps 0.7 title "EF=50", \\
EOF
        ...
        for ef in "${ef_values[@]}"; do
            if [ $col -le $(cat ${data_dir}/search.dat | head -1 | wc -w) ]; then
                echo "     \"${data_dir}/search.dat\" using 1:${col} with linespoints pt 7 ps 0.7 title \"EF=${ef}\", \\" >> "${output_dir}/gnuplot_search.gp"
                col=$((col + 1))
            fi
        done
...
# Get number of columns for EF values
ncols = system("awk 'NR==1 {print NF-1}' ${data_dir}/insertion.dat")
...
# Fit the curve using the last EF value
fit_col = ncols + 1
stats "${data_dir}/insertion.dat" using 1:fit_col nooutput
c = column(fit_col)[STATS_records] / log(STATS_max_x)

plot "${data_dir}/insertion.dat" using 1:2 with linespoints pt 7 ps 0.7 title "EF=50", \\
EOF
        ...
        for ef in "${ef_values[@]}"; do
            if [ $col -le $(cat ${data_dir}/insertion.dat | head -1 | wc -w) ]; then
                echo "     \"${data_dir}/insertion.dat\" using 1:${col} with linespoints pt 7 ps 0.7 title \"EF=${ef}\", \\" >> "${output_dir}/gnuplot_insertion.gp"
                col=$((col + 1))
            fi
        done
...
# Get number of columns for EF values
ncols = system("awk 'NR==1 {print NF-1}' ${data_dir}/total_insertion.dat")
...
# Fit the curve using the last EF value
fit_col = ncols + 1
stats "${data_dir}/total_insertion.dat" using 1:fit_col nooutput
d = column(fit_col)[STATS_records] / (STATS_max_x * log(STATS_max_x))

plot "${data_dir}/total_insertion.dat" using 1:2 with linespoints pt 7 ps 0.7 title "EF=50", \\
EOF
        ...
        for ef in "${ef_values[@]}"; do
            if [ $col -le $(cat ${data_dir}/total_insertion.dat | head -1 | wc -w) ]; then
                echo "     \"${data_dir}/total_insertion.dat\" using 1:${col} with linespoints pt 7 ps 0.7 title \"EF=${ef}\", \\" >> "${output_dir}/gnuplot_total_insertion.gp"
                col=$((col + 1))
            fi
        done
    ...
    echo "Creating combined PDF report for M=${m_value}..."
    pdftk "${output_dir}/construction_time_m${m_value}.pdf" \
          "${output_dir}/search_time_m${m_value}.pdf" \
          "${output_dir}/insertion_time_m${m_value}.pdf" \
          "${output_dir}/total_insertion_time_m${m_value}.pdf" \
          cat output "${output_dir}/combined_report_m${m_value}.pdf"
...
create_m_comparison_plots() {
    ...
    local dim=$(grep "DIM" "${first_dir}/parameters.dat" | awk '{print $2}')
...
EOF
    ...
    for m in "${m_values[@]}"; do
        echo "    \"m_analysis/gnuplot_data/m_${m}/construction.dat\" using 1:3 with linespoints pt 7 ps 0.7 title \"M=${m}\", \\" >> "${output_dir}/m_comparison_construction.gp"
    done
...
EOF
    ...
    # Add each M value to the plot
    for m in "${m_values[@]}"; do
        echo "    \"m_analysis/gnuplot_data/m_${m}/search.dat\" using 1:3 with linespoints pt 7 ps 0.7 title \"M=${m}\", \\" >> "${output_dir}/m_comparison_search.gp"
    done
...
EOF
    ...
    # Add each M value to the plot
    for m in "${m_values[@]}"; do
        echo "    \"m_analysis/gnuplot_data/m_${m}/insertion.dat\" using 1:3 with linespoints pt 7 ps 0.7 title \"M=${m}\", \\" >> "${output_dir}/m_comparison_insertion.gp"
    done
    ...
    # Create a combined M comparison PDF report
    echo "Creating combined M comparison PDF report..."
    pdftk "${output_dir}/m_comparison_construction.pdf" \
          "${output_dir}/m_comparison_search.pdf" \
          "${output_dir}/m_comparison_insertion.pdf" \
          cat output "${output_dir}/m_comparison_report.pdf"
...
m_values=$(find output/m_analysis/data -maxdepth 1 -type d -name "m_*" | sort -V | sed 's/.*m_//')
...
echo "Note: To use the pdftk commands for combining PDFs, you may need to install it with: sudo apt-get install pdftk" 