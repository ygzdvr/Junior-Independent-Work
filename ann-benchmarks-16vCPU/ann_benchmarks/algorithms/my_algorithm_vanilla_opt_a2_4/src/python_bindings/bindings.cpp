 ...
 public:
    ...
    void set_pruning_alpha(float alpha) {
        if (appr_alg) {
            ...
            // Throw error if called before init_index creates appr_alg
            throw std::runtime_error("Index not initialized. Cannot set pruning alpha.");
        }
    }
...
PYBIND11_PLUGIN(hnswlib) {
        ...
        .def("get_pruning_alpha", &Index<float>::getPruningAlpha)
        .def_property("ef",
          ...
          [](Index<float> & index, const size_t ef_) {
            ...
            if (index.appr_alg)
              index.appr_alg->setEf(ef_); // Use the setter method
        })
        ...
        .def_property_readonly("pruning_alpha",  [](const Index<float> & index) {
          return index.index_inited ? index.appr_alg->getPruningAlpha() : 1.0f; // Return default if not inited
        })
