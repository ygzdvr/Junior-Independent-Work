...
class Index {
 public:
    ...
    void init_new_index(
        ...
        float pruning_alpha = 1.2f) {
        if (appr_alg) {
            throw std::runtime_error("The index is already initiated.");
        }
        cur_l = 0;
        appr_alg = new hnswlib::HierarchicalNSW<dist_t>(l2space, maxElements, M, efConstruction, random_seed, allow_replace_deleted, pruning_alpha);
        index_inited = true;
        ep_added = false;
    ...
    void set_pruning_alpha(float alpha) {
      if (appr_alg)
          appr_alg->setPruningAlpha(alpha);
    }
    ...
    py::dict getAnnData() const { /* WARNING: Index::getAnnData is not thread-safe with Index::addItems */
        ...
        py::dict d = py::dict(
            ...
            "max_level"_a = appr_alg->maxlevel_.load(),
            "enterpoint_node"_a = appr_alg->enterpoint_node_.load(),
            ...
            "pruning_alpha"_a = appr_alg->getPruningAlpha(),
        ...
        // --- Add compression state ---\n         d[\"use_compression\"] = appr_alg->use_compression_;\n         if (appr_alg->use_compression_ && appr_alg->pq_) {\n             d[\"pq_m\"] = appr_alg->pq_->m;\n             d[\"pq_kbits\"] = appr_alg->pq_->kbits;\n             // Serialize centroids (and rotation matrix if OPQ)\n             // This requires converting vector<float> to numpy array\n             d[\"pq_centroids\"] = py::array_t<float>(py::cast(appr_alg->pq_->centroids));\n             if(appr_alg->pq_->opq_trained){\n                  d[\"pq_rotation\"] = py::array_t<float>(py::cast(appr_alg->pq_->rotation_matrix));\n             }\n             // Serialize compressed codes\n             py::capsule free_compressed(appr_alg->compressed_codes_memory_, [](void* f) { /* Don't free here, owned by appr_alg */ });\n             d[\"compressed_codes\"] = py::array_t<char>(\n                { (size_t)appr_alg->cur_element_count, appr_alg->pq_code_size_ }, \n                { appr_alg->pq_code_size_ * sizeof(char), sizeof(char) }, \n                appr_alg->compressed_codes_memory_, \n                free_compressed); \n        }\n        // --- End compression state ---\n        return d;\n    }\n\n    void setAnnData(const py::dict d) { \n        // ... Existing deserialization ...\n        std::unique_lock <std::mutex> templock(appr_alg->global);\n        // ... Load base HNSW data ...\n        \n        // --- Load compression state --- \n        if (d.contains(\"use_compression\") && d[\"use_compression\"].cast<bool>()) {\n            size_t pq_m = d[\"pq_m\"].cast<size_t>();\n            size_t pq_kbits = d[\"pq_kbits\"].cast<size_t>();\n            \n            appr_alg->pq_ = std::make_unique<ProductQuantizer>(appr_alg->data_size_, pq_m, pq_kbits);\n            appr_alg->pq_code_size_ = appr_alg->pq_->code_size;\n\n            // Load centroids\n            py::array_t<float, py::array::c_style | py::array::forcecast> centroids_arr = d[\"pq_centroids\"].cast<py::array_t<float>>();\n            appr_alg->pq_->centroids = std::vector<float>(centroids_arr.data(), centroids_arr.data() + centroids_arr.size());\n            \n            // Load rotation matrix if exists (OPQ)\n            if (d.contains(\"pq_rotation\")) {\n                 py::array_t<float, py::array::c_style | py::array::forcecast> rotation_arr = d[\"pq_rotation\"].cast<py::array_t<float>>();\n                 appr_alg->pq_->rotation_matrix = std::vector<float>(rotation_arr.data(), rotation_arr.data() + rotation_arr.size());\n                 appr_alg->pq_->opq_trained = true;\n            }\n\n            // Load compressed codes\n            py::array_t<char, py::array::c_style | py::array::forcecast> codes_arr = d[\"compressed_codes\"].cast<py::array_t<char>>();\n            appr_alg->compressed_codes_memory_ = (char*)malloc(codes_arr.nbytes());\n            if (!appr_alg->compressed_codes_memory_) throw std::runtime_error(\"Failed to allocate memory for compressed codes during load\");\n            memcpy(appr_alg->compressed_codes_memory_, codes_arr.data(), codes_arr.nbytes());\n            \n            appr_alg->use_compression_ = true;\n            std::cout << \"Loaded index with PQ compression (m=\" << pq_m << \", kbits=\" << pq_kbits << \")\" << std::endl;\n        }\n        // --- End compression state ---\n         // ... Load rest of HNSW data (links etc.) ...\n    }\n    // ... (createFromParams needs similar logic to handle optional compression fields) ...\n};\n\n// --- PYBIND11 Module Definition ---\nPYBIND11_PLUGIN(hnswlib) {\n    py::module m(\"hnswlib\");\n\n    py::class_<Index<float>>(m, \"Index\")\n        // ... existing .def calls (init, knn_query, add_items, etc.) ...\n        .def(\"init_index\",\n            &Index<float>::init_new_index,\n            py::arg(\"max_elements\"),\n            py::arg(\"M\") = 16,\n            py::arg(\"ef_construction\") = 200,\n            py::arg(\"random_seed\") = 100,\n            py::arg(\"allow_replace_deleted\") = false,\n            py::arg(\"pruning_alpha\") = 1.2f)\n            \n        // >>> Add PQ training binding <<< 
        .def(\"train_pq\", &Index<float>::train_pq, \n             py::arg(\"train_data\"), \n             py::arg(\"pq_m\"), \n             py::arg(\"pq_kbits\"))\n             \n        .def(\"knn_query\",\n             &Index<float>::knnQuery_return_numpy,\n            py::arg(\"data\"),\n            py::arg(\"k\") = 1,\n            py::arg(\"num_threads\") = -1,\n            py::arg(\"filter\") = py::none())\n        .def(\"add_items\",\n            &Index<float>::addItems,\n            py::arg(\"data\"),\n            py::arg(\"ids\") = py::none(),\n            py::arg(\"num_threads\") = -1,\n            py::arg(\"replace_deleted\") = false)\n        .def(\"get_items\", &Index<float>::getData, py::arg(\"ids\") = py::none(), py::arg(\"return_type\") = \"numpy\")\n        .def(\"get_ids_list\", &Index<float>::getIdsList)\n        .def(\"set_ef\", &Index<float>::set_ef, py::arg(\"ef\"))\n        .def(\"set_num_threads\", &Index<float>::set_num_threads, py::arg(\"num_threads\"))\n        .def(\"index_file_size\", &Index<float>::indexFileSize)\n        .def(\"save_index\", &Index<float>::saveIndex, py::arg(\"path_to_index\"))\n        .def(\"load_index\",\n            &Index<float>::loadIndex,\n            py::arg(\"path_to_index\"),\n            py::arg(\"max_elements\") = 0,\n            py::arg(\"allow_replace_deleted\") = false)\n        .def(\"mark_deleted\", &Index<float>::markDeleted, py::arg(\"label\"))\n        .def(\"unmark_deleted\", &Index<float>::unmarkDeleted, py::arg(\"label\"))\n        .def(\"resize_index\", &Index<float>::resizeIndex, py::arg(\"new_size\"))\n        .def(\"get_max_elements\", &Index<float>::getMaxElements)\n        .def(\"get_current_count\", &Index<float>::getCurrentCount)\n        .def_readonly(\"space\", &Index<float>::space_name)\n        .def_readonly(\"dim\", &Index<float>::dim)\n        .def_readwrite(\"num_threads\", &Index<float>::num_threads_default)\n        .def(\"set_pruning_alpha\", &Index<float>::set_pruning_alpha, py::arg(\"alpha\"))\n        .def_property(\"ef\",\n          [](const Index<float> & index) {\n            return index.index_inited ? index.appr_alg->ef_ : index.default_ef;\n          },\n          [](Index<float> & index, const size_t ef_) {\n            index.default_ef = ef_;\n            if (index.appr_alg)\n              index.appr_alg->ef_ = ef_;\n        })\n        .def_property_readonly(\"max_elements\", [](const Index<float> & index) {\n            return index.index_inited ? index.appr_alg->max_elements_ : 0;\n        })\n        .def_property_readonly(\"element_count\", [](const Index<float> & index) {\n            return index.index_inited ? (size_t)index.appr_alg->cur_element_count : 0;\n        })\n        .def_property_readonly(\"ef_construction\", [](const Index<float> & index) {\n          return index.index_inited ? index.appr_alg->ef_construction_ : 0;\n        })\n        .def_property_readonly(\"M\",  [](const Index<float> & index) {\n          return index.index_inited ? index.appr_alg->M_ : 0;\n        })\n        .def_property_readonly(\"pruning_alpha\",  [](const Index<float> & index) {\n          return index.index_inited ? index.appr_alg->getPruningAlpha() : 1.2f;\n        })\n        // >>> Add compression properties <<< 
        .def_property_readonly(\"is_compressed\", [](const Index<float>& index) {\n            return index.appr_alg ? index.appr_alg->use_compression_ : false;\n        })\n        .def_property_readonly(\"pq_m\", [](const Index<float>& index) {\n            return (index.appr_alg && index.appr_alg->pq_) ? index.appr_alg->pq_->m : 0;\n        })\n        .def_property_readonly(\"pq_kbits\", [](const Index<float>& index) {\n            return (index.appr_alg && index.appr_alg->pq_) ? index.appr_alg->pq_->kbits : 0;\n        })\n        \n        .def(py::pickle(\n             [](const Index<float> &ind) { /* ... existing __getstate__ ... */ return py::make_tuple(ind.getIndexParams()); },\n             [](py::tuple t) { /* ... existing __setstate__ ... */ if(t.size()!=1) throw std::runtime_error(\"Invalid state!\"); return Index<float>::createFromParams(t[0].cast<py::dict>()); }\n        ));\n        \n    // ... BFIndex ...\n    return m.ptr();\n}\n
    ...
    static Index<float>* createFromParams(const py::dict d) {
        ...
        if (index_inited_) {
            // Check if pruning_alpha exists in the dictionary
            float alpha = 1.2f; // Default value
            if (d.contains("pruning_alpha")) {
                alpha = d["pruning_alpha"].cast<float>();
            }
            bool allow_replace_deleted = false; // Default value
            ...
            new_index->appr_alg = new hnswlib::HierarchicalNSW<dist_t>(
                ...
                alpha // Pass the loaded or default alpha
            );
    ...
    void setAnnData(const py::dict d) { /* WARNING: Index::setAnnData is not thread-safe with Index::addItems */
        ...
        appr_alg->maxlevel_.store(d["max_level"].cast<int>());
        appr_alg->enterpoint_node_.store(d["enterpoint_node"].cast<hnswlib::tableint>());
        ...
        if (d.contains("pruning_alpha")) {
            appr_alg->setPruningAlpha(d["pruning_alpha"].cast<float>());
        }
    ...
    float getPruningAlpha() const {
        return appr_alg->getPruningAlpha();
    }

    // --- Add training method implementation ---
    void train_pq(py::object train_data_obj, size_t pq_m, size_t pq_kbits) {
        if (!appr_alg) {
            throw std::runtime_error("Index must be initialized before training quantizer.");
        }
        
        py::array_t<float, py::array::c_style | py::array::forcecast> train_data_arr(train_data_obj);
        auto buffer = train_data_arr.request();
        
        size_t rows, features;
        get_input_array_shapes(buffer, &rows, &features);

        if (features != dim) {
            throw std::runtime_error("Training data dimensionality does not match index dimension.");
        }
        if (rows == 0) {
             throw std::runtime_error("Training data cannot be empty.");
        }
        
        // Call the actual implementation in HierarchicalNSW
        appr_alg->trainQuantizer(static_cast<const float*>(buffer.ptr), rows, pq_m, pq_kbits);
    }
    // --- End training method implementation ---
};
