...
"""
    ...
    # Defensive check for unexpected argument list length
    if isinstance(algo_args, list) and len(algo_args) > 2:
        print(f"Warning: Received {len(algo_args)} arguments, expected 2. Using only the first two: {algo_args[:2]}")
        algo_args = algo_args[:2]
