...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    size_t ef_{ 0 };
    float eps_rad_{0.1f};  // Angular tolerance parameter for diversity
    ...
    void getNeighborsByHeuristic2(
        ...
        const size_t M);
    /**
     * Build an angularly diverse neighbour set for node 'v'.
     *
     * @param v            internal id of the node whose neighbours we pick
     * @param candidates   vector of {distance_to_v, internal_id} produced
     *                     by efConstruction (unsorted order is fine)
     * @param M            hard degree cap
     * @param eps_rad      angular tolerance in *radians*  (e.g. 0.52 ≈ 30°)
     * @param pool         temporary buffer reused by the builder
     * @return             chosen internal ids  (size ≤ M)
     */
    void selectAngularDiverseNeighbors(tableint v,
                                      std::vector<std::pair<dist_t,tableint>>& candidates,
                                      size_t M,
                                      float eps_rad,
                                      std::vector<tableint>& pool);
                                   
    // fast helper: cosine of angle between vectors v→a and v→b
    float cosFromV(tableint v, tableint a, tableint b) const;
