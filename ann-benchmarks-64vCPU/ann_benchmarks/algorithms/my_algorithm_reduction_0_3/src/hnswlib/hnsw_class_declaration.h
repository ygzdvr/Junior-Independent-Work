...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    size_t level0_dim_{0};                      // original dimension
    std::vector<size_t> proj_dim_;              // projected dimensions per level
    std::vector<std::vector<float>> proj_mat_;  // projection matrices per level
    std::vector<std::vector<float>> proj_data_; // projected data per level
    ...
    void initProjections(size_t max_level, float target_eps = 0.10f, size_t min_dim = 32);
    void storePointAcrossLayers(tableint id, const float* raw);
    inline dist_t layerDist(tableint a, tableint b, size_t level) const;
